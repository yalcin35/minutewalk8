...
import { p as push, d as set, o as onMount, l as legacy_pre_effect, a as legacy_pre_effect_reset, t as template_effect, b as pop, c as child, s as sibling, g as get, h as derived_safe_equal, e as mutate, m as mutable_source, r as reset, n as next, u as update, f as first_child } from "../chunks/BXHzHFYX.js";
...
var root_8 = template(`<button class="w-12 h-12 rounded-xl bg-white text-indigo-500 shadow-lg
...
var root_12 = template(`<button class="w-12 h-12 rounded-xl bg-indigo-500 text-white shadow-lg
               flex items-center justify-center transform transition-all
               duration-200 hover:scale-105 active:scale-95
               focus:outline-none focus:ring-2 focus:ring-indigo-500
               focus:ring-offset-2"><!></button>`);
...
var root = template(`<div class="min-h-screen bg-gradient-to-br from-indigo-50 to-white"><div class="fixed top-0 left-0 w-full h-1 bg-gray-100"><div class="h-full bg-indigo-500 transition-all duration-300 ease-out"></div></div> <button class="fixed top-4 right-4 flex items-center space-x-2 px-4 py-2 bg-red-50 
           text-red-600 rounded-lg hover:bg-red-100 transition-colors z-50"><!> <span>Exit</span></button> <div class="bg-white shadow-sm"><div class="max-w-4xl mx-auto px-6 py-4 flex items-center"><div class="flex-1"><h1 class="text-sm font-medium text-indigo-600"> </h1> <p class="text-gray-500 text-sm mt-1"> </p></div> <div class="flex-1 flex justify-center"><div class="flex items-center space-x-2 bg-indigo-50 px-3 py-1.5 rounded-lg"><!></div></div> <div class="flex-1"></div></div></div> <div class="max-w-4xl mx-auto p-6 space-y-8"><div class="space-y-8"><div class="bg-white rounded-2xl shadow-lg p-6 space-y-6"><div class="flex items-start space-x-4"><div class="bg-indigo-100 p-2 rounded-full flex-shrink-0 mt-1"><!></div> <h2 class="text-xl font-light text-gray-900"> </h2></div> <!> <div class="flex justify-between items-center py-4"></div> <div class="space-y-2"><label class="text-sm font-medium text-gray-700">Evidence Photos</label> <!> <!></div> <div class="space-y-2"><label class="text-sm font-medium text-gray-700"> </label> <textarea class="w-full p-4 h-24 rounded-xl bg-gray-50
...
function _page($$anchor, $$props) {
  ...
  onMount(async () => {
    try {
      const { data: setupData, error: setupError } = await supabase.from("audit_setups").select(`
          ...
          audit_type,
          start_time
        `).eq("audit_type", "gemba").order("created_at", { ascending: false }).limit(1).single();
  ...
  async function requestAIInsights() {
    ...
    set(aiError, null);
    try {
      const insight = await getAIInsights(get(currentQuestion).text, get(currentAnswer).rating, get(currentAnswer).notes, get(currentAnswer).photos.map((p) => p.file), "gemba");
  ...
  function navigate(direction) {
    ...
    } else if (direction === "next" && get(currentQuestionIndex) < gembaQuestions.length - 1) {
  ...
  legacy_pre_effect(
    ...
    () => {
      set(currentQuestion, gembaQuestions[get(currentQuestionIndex)]);
    }
  );
  legacy_pre_effect(
  ...
  legacy_pre_effect(
    () => (get(currentQuestionIndex), gembaQuestions),
    () => {
      progress.set(get(currentQuestionIndex) / (gembaQuestions.length - 1) * 100);
    }
  );
  ...
  Orbit(node_2, { class: "w-5 h-5 text-indigo-600" });
  reset(div_12);
  ...
  each(div_13, 4, () => Array(5), index, ($$anchor2, _, i, $$array) => {
    ...
    template_effect(() => set_class(button_1, 1, `w-14 h-14 rounded-xl transition-all duration-200
                     ...
                     ${(get(currentAnswer).rating === i + 1 ? "bg-indigo-500 text-white shadow-lg" : "bg-white shadow-md") ?? ""}`));
  ...
  var node_14 = sibling(node_12, 2);
  {
    ...
    if_block(node_14, ($$render) => {
      if (get(currentQuestionIndex) === gembaQuestions.length - 1) $$render(consequent_7);
  ...
  template_effect(
    ($0, $1) => {
      ...
      set_text(text_1, `Question ${get(currentQuestionIndex) + 1} of ${gembaQuestions.length ?? ""}`);
...
//# sourceMappingURL=6.CD3HXVH6.js.map
