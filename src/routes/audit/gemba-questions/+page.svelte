<script lang="ts">
  ...
  import { ChevronRight, ChevronLeft, Save, Brain, RefreshCw, Circle as XCircle, ArrowLeft, Clock, Orbit } from 'lucide-svelte';
  ...
  $: currentQuestion = gembaQuestions[currentQuestionIndex];
  $: currentAnswer = answers[currentQuestion.id] || { rating: 0, photos: [], notes: '', aiInsight: null };
  $: progress.set((currentQuestionIndex / (gembaQuestions.length - 1)) * 100);
  ...
  async function requestAIInsights() {
    ...
    try {
      const insight = await getAIInsights(
        ...
        'gemba' // Pass 'gemba' as the audit type to use the Gemba-specific endpoint
      );
  ...
  function navigate(direction: 'prev' | 'next') {
    ...
    } else if (direction === 'next' && currentQuestionIndex < gembaQuestions.length - 1) {
      currentQuestionIndex++;
    }
    if ('vibrate' in navigator) navigator.vibrate(15);
    updateHistory();
...
<div class="min-h-screen bg-gradient-to-br from-indigo-50 to-white">
    ...
    <div class="max-w-4xl mx-auto px-6 py-4 flex items-center">
      ...
      <div class="flex-1">
        ...
        <p class="text-gray-500 text-sm mt-1">
          Question {currentQuestionIndex + 1} of {gembaQuestions.length}
        </p>
      </div>

      <!-- Centered Timer -->
      <div class="flex-1 flex justify-center">
        <div class="flex items-center space-x-2 bg-indigo-50 px-3 py-1.5 rounded-lg">
          <AuditTimer 
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 space-y-6">
        <div class="flex items-start space-x-4">
          ...
          <h2 class="text-xl font-light text-gray-900">
            {currentLocale === 'de' && currentQuestion.text_de ? currentQuestion.text_de :
             currentLocale === 'tr' && currentQuestion.text_tr ? currentQuestion.text_tr : currentQuestion.text}
          </h2>
        </div>
          ...
          {#each Array(5) as _, i}
            <button
              class="w-14 h-14 rounded-xl transition-all duration-200
                     ...
                     {currentAnswer.rating === i + 1 ? 'bg-indigo-500 text-white shadow-lg' : 'bg-white shadow-md'}"
  ...
  <div class="fixed bottom-8 right-8 flex space-x-4">
    {#if currentQuestionIndex > 0}
      <button
        class="w-12 h-12 rounded-xl bg-white text-indigo-500 shadow-lg
    ...
    {#if currentQuestionIndex === gembaQuestions.length - 1}
      <button
    ...
    {:else}
      <button
        class="w-12 h-12 rounded-xl bg-indigo-500 text-white shadow-lg
               flex items-center justify-center transform transition-all
               duration-200 hover:scale-105 active:scale-95
               focus:outline-none focus:ring-2 focus:ring-indigo-500
               focus:ring-offset-2"
